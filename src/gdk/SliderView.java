...
/**
 * Slider view , used in the modules list by swipe
 * @author guillaumeagis
 *
 */
public class SliderView extends FrameLayout
{
    private float _animatedCount = 0.0F;
    private int _count = 0;
    private ObjectAnimator _countAnimator;

    private Runnable _hideSliderRunnable = new Runnable()
    {
        public void run()
        {
    ...
    {
        if ((this._countAnimator != null) && (this._countAnimator.isRunning()))
            this._countAnimator.cancel();

        float[] arrayOfFloat = new float[2];
        arrayOfFloat[0] = this._animatedCount;
        arrayOfFloat[1] = paramFloat;
        this._countAnimator = ObjectAnimator.ofFloat(this, "animatedCount", arrayOfFloat);
        this._countAnimator.setDuration(300L);
        this._countAnimator.start();
    }

    private int getBaseSliderWidth()
    {
        return Math.max((int)(getResources().getDisplayMetrics().widthPixels / this._animatedCount), 40);
    }
    ...
    {
        removeCallbacks(this._hideSliderRunnable);
        postDelayed(this._hideSliderRunnable, 2000L);
    }
    ...
    private void setProportionalIndex(float paramFloat, int paramInt, boolean paramBoolean)
    {
        ...
        while (true)
        {
            ...
            float f2 = (0.5F + this.index - f1 / 2.0F) * (getResources().getDisplayMetrics().widthPixels / this._count);
            if (paramInt != 0)
    ...
    {
        this._animatedCount = paramFloat;
